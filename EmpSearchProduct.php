...
if (isset($_POST["submit"])) {
    ...
    if (empty($pID) && empty($prodName)) {
        echo "<script>alert('Please enter a product ID and/or product name.')</script>";
    }
    if (empty($pID) && !empty($prodName)) {
        require_once("db.php");
        $sql = "select PID, Pname, PDesc, Price from bit4444group41.product where Pname = '$prodName'";
    ...
    if (!empty($pID) && empty($prodName)) {
        require_once("db.php");
        $sql = "select Pname, PDesc, Price from product where PID = $pID";
        $result = $mydb -> query($sql);
        $row = mysqli_fetch_array($result);
        if ($row) {
            $prodName = $row["Pname"];
    ...
    if (!empty($pID) && !empty($prodName)) {
        require_once("db.php");
        $sql = "select Pname, PDesc, Price from product where Pname = '$prodName' and PID = $pID";
        $result = $mydb -> query($sql);
        $row = mysqli_fetch_array($result);
        if ($row) {
            $prodName = $row["Pname"];
